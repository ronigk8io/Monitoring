...
BlockJinn scanners (app-level lag -- separate from node MonitorManager):
- Script: `check_blockjinn_scanners.py` -- probes all mainnet `https://blockjinn.gk8.network/<chain>/v1/health` milestone ages
- Companion: `check_polymesh_faces.py` -- Polymesh Hot-like + node face probes
- Safe test: `python3 check_blockjinn_scanners.py --dry-run`
- Test Slack: `python3 check_blockjinn_scanners.py --channel-test --max-age-sec 1 --chains ethereum --force-notify`
- Prod cron (Monitoring host `ubuntu@3.222.94.21`, log `~/logs/blockjinn_scanners.log`):
  `*/5 * * * * cd /home/ubuntu/Monitoring && /usr/bin/python3 check_blockjinn_scanners.py >> ~/logs/blockjinn_scanners.log 2>&1`
